...
export default class Action extends SAction {
    ...
    getAllHistory(extra?: {}) {
        ...
        const data = reducer?.data_usuario;
        if (!data) {
    ...
    getAllActivosInicio(extra?: {}) {
        var reducer = this._getReducer();
        const data = reducer?.data_activos_inicio;
        if (!data) {
            if (reducer.estado == "cargando") return null;
            const petition = {
                ...this.model.info,
    ...
    getEnCurso() {
        const data = this.getAllActivosInicio()
        if (!data) return null;
    ...
    getQR({ key_pedido }) {
        var content = `tapeke://pedido/${key_pedido}`;
