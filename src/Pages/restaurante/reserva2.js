...
class index extends Component {
        ...
        return <SView width={114} center row border={'transparent'}  >
            <SView col={"xs-12"} center>
                <SView width={114} height={26} center style={{ borderRadius: 8, backgroundColor: STheme.color.primary }}>
                    <SText fontSize={10} color={STheme.color.secondary} >{this.data?.proximo_horario?.pack?.cantidad_disponibles ?? 0} disponible(s)</SText>
                </SView>
            </SView>
                ...
                <SView col={"xs-3"} center border={'transparent'} >
                    <SView width={36} height={36} center style={{ backgroundColor: STheme.color.primary, borderRadius: 17 }}
                        onPress={() => {
                            ...
                            if (this.state.cantidad >= this.data?.capacidad_envio && !!this.state.delivery) {
            ...
            onPress: () => {
                if (this.costo_envio) {
                    if (this.costo_envio.monto) {
                        if (this.state.cantidad >= this.data?.capacidad_envio) {
                            this.state.cantidad = this.data?.capacidad_envio
                            SPopup.alert("Alcanzaste la capacidad máxima de delivey.")
                        }
                        let incentivos = this.getTotalIncentivos();
                        this.setState({ envio: this.costo_envio.monto + incentivos, delivery: "true" })
                    }
                }
            }
        } : {})}>
    ...
    getCostoEnvio() {
        ...
        if (costo.monto) {
            ...
            return <SText fontSize={14}   >Costo del envío: Bs. {SMath.formatMoney(costo.monto + incentivos)} </SText>
        } else {
    ...
    getMontoTotal() {
        let _number = (this.state.cantidad * (this.data?.proximo_horario?.pack?.precio ?? 0)) + this.state.envio;
        return _number;
    }
    ...
    render_data() {
        ...
        this.totalIncentivos = this.getTotalIncentivos()
            ...
            <SView col={"xs-12"} height backgroundColor={STheme.color.card} style={{
                ...
                <SView col={"xs-12 sm-10 md-8 lg-6 xl-4"} row center style={{ backgroundColor: STheme.color.white }}>
                    <SView col={"xs-11"} row center>
                        ...
                        <SView col={"xs-6"} style={{ alignItems: "flex-end" }}>
                            <SText fontSize={15} style={{ fontWeight: "bold" }} >Bs. {this.getMontoTotal()}</SText>
                        </SView>
                        <SHr height={15} />
                    </SView>
                </SView>
    ...
    getTotalIncentivos() {
        // if (!this.state.delivery) return 0;
        this.incentivos = Model.incentivo.Action.getAllActivos({ key_restaurante: this.pk });
        if (!this.incentivos) return 0;
        let monto = 0;
    ...
    getIncentivos() {
        ...
        this.incentivos = Model.incentivo.Action.getAllActivos({ key_restaurante: this.pk });
        if (!this.state.delivery) return null;
        if (!this.incentivos) return <SLoad />
        return Object.values(this.incentivos).map((obj) => {
