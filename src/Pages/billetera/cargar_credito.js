...
class index extends Component {
        ...
        return <>
            <SView col={"xs-10"} row center   >
                ...
                <PButtom fontSize={20} loading={this.state.loading} onPress={() => {
                    ...
                    if (parseFloat(this.input.getValue()) < 1){
                        SPopup.alert("El valor debe ser mayor a 1 Bs.")
                    ...
                    if (!tipoPago) {
                        SPopup.alert("Seleccione un método de pago")
                        return null;
                    }
                    ...
                    SSocket.sendPromise(
                        ...
                        switch (tipoPago.key) {
                            case "QR":
                                SNavigation.replace("/billetera/qr", { data: JSON.stringify(resp.data) })
                                break;
                            case "Credito":
    ...
    render() {
        return (<SPage title={''} onRefresh={() => { Model.billetera.Action.CLEAR(); }} >
            ...
            <SLoad type='window' label='Verificando método de pago' hidden={!this.state.loading} />
        </SPage>
        );
    }
}
