...
export default class listo extends Component {
        ...
        return arr.map((x, i) => {
            return <>
                <SView col={"xs-2.2"} height={40} row center onPress={() => {
                    ...
                    <SIcon name={i > this.state.selectValue ? "Estrella2Off" : "Estrella2On"} width={i > this.state.selectValue ? 40 : 40} fill={STheme.color.card} />
                </SView>
            </>
        })
    }
    ...
    calificar() {
        ...
        Model.calificacion.Action.registro({
            data: obj,
            key_restaurante: this.props.data.restaurante.key,
    ...
    render() {
        return (<SView col={"xs-12"} backgroundColor={STheme.color.card} height center >
            <Container>
                <SHr height={20} />
                <SView col={"xs-12"} row center >
                    <SView col={"xs-12"} backgroundColor={STheme.color.primary} style={{ borderRadius: 16 }} center >
                        <SView col={"xs-11"} center >
                            <SHr height={50} />
                            <SText fontSize={32} color={STheme.color.secondary} bold center>¿Cómo calificarías el Tapeke?</SText>
                            <SHr height={5} />
